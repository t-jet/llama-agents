...
Action:
- Check live entry docs, stage design entry docs, active fix reports, correction-evidence status lines, correction part handoff sections, downstream design handoff sections, `document-index.md` design and implementation rows, top-level Status lines, current-status sections, handoff text, and linked gate-status part files before and after patching or reviewing. Distinguish historical quoted findings from current contradictions. Keep durable gate-status locations in same state: reviewable, rework-required, manager-gate-ready, planning-open, approval-pending, approved, ready-for-QA, bug-fix-review-pass, implementation-re-review-pass, or blocked. Don't leave stale limitation, review-pending, awaiting-review, re-review-ready, handoff-closed, ready-for-review, ready-for-implementation, ready-for-re-review, or not-started wording after gate has advanced or while open finding remains; in implementation reviews, flag stale design-entry or index gate text when it contradicts implementation log state.
...
## Improvement: Windows path resolution treats `_design_docs` and `._design_docs` as same directory

Condition:
- Creating or moving files under `._design_docs/` on Windows, where `create_file` and `replace_string_in_file` tools strip or normalize the leading dot, and PowerShell `Test-Path`/`Get-ChildItem` resolve both spellings to the same physical directory because Windows file system ignores leading dots

Action:
- After creating or moving a new part file, run `git status -- <dir>/` and confirm git reports the new file at the dot-prefix path used by the rest of the tree. If git shows the file at a different spelling or under a wrong leaf directory, use `Move-Item -Path <src> -Destination <dst>` to relocate it before declaring work done. Verify the destination exists at the dot-prefix path and the source is gone. Don't rely on `Test-Path` returning true under both spellings — both spellings can resolve to the same Windows path while git's view still uses the dot prefix. Don't accept "file exists" as proof the new file lives where durable navigation expects it.
...
## Improvement: Persistent stage designs cite durable stage state, not test reports

Condition:
- Authoring persistent design docs from a manager close-at-current-progress report or other one-time test report

Action:
- Use the test report as source evidence while drafting, but cite durable stage state through the prior implementation log, architecture part, or manager handoff wording in the new design. Don't make the new persistent design doc depend on a specific `test-report-*.md` path as required context unless the user explicitly asks for that exact report link in the design.

## Improvement: Endpoint parity plan reviews trace converted routes and namespace fields

Condition:
- Reviewing endpoint compatibility or endpoint parity implementation plan against live server route wiring and cache metadata namespace rules

Action:
- Trace every registered public route that converts into an in-scope handler or `SERVER_TASK_TYPE_COMPLETION`, even if docs label it optional or fixture-dependent. Also check excluded prompt-like routes such as infill, control, token-count, rerank, and embeddings against the live task type and cache admission gate before treating omission as a blocker. Check whether any planned metadata field, especially `preparation_id` or `degraded_reason`, feeds `compute_namespace_id`; flag route-specific values in namespace-affecting fields as blockers unless plan clearly separates diagnostic-only source labels from cache namespace identity.

## Improvement: Split cache controller accessor reviews

Condition:
- Reviewing a bug fix that adds a new accessor or helper to avoid an assertion, checksum mismatch, namespace mismatch, or cache-identity bug in a controller split across multiple implementation files

Action:
- Search the whole controller and compiled server target for the old accessor or helper, not only the files named in the fix report. Verify every live admission, lookup, prefix-index, branch-node, checksum, save, restore, and test-helper path uses the new accessor where the fixed data shape can flow. Flag remaining old-accessor use as blocking when it can still receive the bug-triggering input. Don't accept a focused accessor unit test as proof that production controller paths were updated.
