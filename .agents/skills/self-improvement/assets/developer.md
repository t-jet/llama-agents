...
Condition:
- Running a PowerShell one-liner that emits objects from `foreach (...) { ... }` and then pipes directly to `Format-Table`, `Select-Object`, or another command

Action:
- Collect objects into an array inside the loop, then pipe the array after the loop. Don't write `foreach (...) { ... } | Format-Table`; PowerShell reports `An empty pipe element is not allowed`, wasting a verification turn.
